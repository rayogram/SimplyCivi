h1. SimplyCivi CSS Framework Tutorial

Welcome to this tutorial on SimplyCivi. It will give you a thorough intro to what you can do with the framework, and a few notes on what you shouldn't do with it. Let's get started.

h2. About SimplyCivi
  
SimplyCivi is a CSS framework, designed to cut down on your development time. It gives you a solid foundation to build your CSS on top of, including some sensible default typography, a customizable grid, a print stylesheet and much more.

However, BP is not a silver bullet, and it's best suited for websites where each page may require its own design. Take a look at existing BP pages before deciding if the framework is right for you. You may also check out the test files in the @tests@ directory, which demonstrates most of the features in SimplyCivi.
...
From the bottom up, here are the CSS layers in SimplyCivi:
...
The second part of SimplyCivi are the scripts, which let you customize most
...
# *Validator*: 	For validating the SimplyCivi core files.

That's the quick overview, so now we can finally get into the details. First, we'll take 
a look at the CSS in SimplyCivi. We'll then move on to the scripts, where I'll show you 
how to customize the framework.

h2. Setting Up SimplyCivi

To use SimplyCivi, you must include three files in your HTML:

* @SimplyCivi/screen.css@:	All CSS for screen, projection viewing.
* @SimplyCivi/print.css@: A basic stylesheet for printing.
* @SimplyCivi/ie.css@:	A few needed corrections for Internet Explorer
...
h2. Using the CSS in SimplyCivi

As mentioned before, there's basically three layers of CSS in SimplyCivi. The first two layers, the browser CSS reset and the default typography, apply themselves by changing the CSS of standard HTML elements. In other words, you don't need to change anything in these files. If you, for instance, want to change the font size, do this in your own stylesheet, so that it's easy to upgrade SimplyCivi when new versions arrive.

h3. Classes for Typography
	
While the typography of SimplyCivi mainly applies itself, there's a few classes
...
To make SimplyCivi style your input elements, each text input element should
...
The third layer is the grid CSS classes, which is the tool SimplyCivi gives you to create almost any kind of grid layout for your site. Keep in mind that most of the CSS behind the grid can be customized (explained below). In this section however, I'm using the default settings.
...
So how do you set up a grid? By using classes provided by SimplyCivi. To create a column, make a new @<div/>@, and apply one of the @.span-x@ classes to it. For instance, if you want a 3-column setup, with two narrow and one wide column, a header and a footer here's how you do it:
...
In addition to the spans, there are two important classes you need to know about. First of all, every SimplyCivi site needs to be wrapped in a div with the class @.container@, which is usually placed right after the body tag. 
...
To create basic grids, this is all you need to know. The grid CSS however, provides many more classes for more intricate designs. To see some of them in action, check out the files in @tests/parts/@. These files demonstrate what's possible with the grid in SimplyCivi.
...
SimplyCivi comes with two scripts: one for compressing and customizing the CSS, and one for validating the core CSS files, which is handy if you're making changes to these files.
...
Note that there are a few validation errors shipping with SimplyCivi. These are known, and comes from a few CSS hacks needed to ensure consistent rendering across the vast browser field.
...
To recompress, you just have to run the script. This will parse the core CSS files and output new compressed files in the SimplyCivi folder. As with the validator, Ruby has to be installed to use this script. In the @lib@ directory, run: @$ruby compress.rb@

Calling this file by itself will pull files from @SimplyCivi/src@ and concatenate them into three files; @ie.css@, @print.css@, and @screen.css@. However, argument variables can be set to change how this works. Calling @$ruby compress.rb -h@ will reveal basic arguments you can pass to the script.
