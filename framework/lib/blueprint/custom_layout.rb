...
module SimplyCivi
  ...
  class CustomLayout
    ...
    CSS_ERB_FILE = File.join(SimplyCivi::LIB_PATH, 'grid.css.erb')

    attr_writer :column_count, :column_width, :gutter_width, :input_padding,
                :input_border

    # Column count of generated CSS.  Returns itself or SimplyCivi's default
    def column_count
      (@column_count || SimplyCivi::COLUMN_COUNT).to_i
    end

    # Column width (in pixels) of generated CSS.  Returns itself or SimplyCivi's default
    def column_width
      (@column_width || SimplyCivi::COLUMN_WIDTH).to_i
    end

    # Gutter width (in pixels) of generated CSS.  Returns itself or SimplyCivi's default
    def gutter_width
      (@gutter_width || SimplyCivi::GUTTER_WIDTH).to_i
    end

    def input_padding
      (@input_padding || SimplyCivi::INPUT_PADDING).to_i
    end

    def input_border
      (@input_border || SimplyCivi::INPUT_BORDER).to_i
    end
    ...
    # Boolean value if current settings are SimplyCivi's defaults
    def default?
      self.column_width == SimplyCivi::COLUMN_WIDTH &&
      self.column_count == SimplyCivi::COLUMN_COUNT &&
      self.gutter_width == SimplyCivi::GUTTER_WIDTH &&
      self.input_padding == SimplyCivi::INPUT_PADDING &&
      self.input_border == SimplyCivi::INPUT_BORDER
    end
