module SimplyCivi
  ...
  class SemanticClassNames
    ...
    #   * <tt>:source_file</tt> -- Source file to use as reference of CSS selectors.  Defaults to SimplyCivi's generated screen.css
    ...
    def initialize(options = {})
      ...
      @source_file = options[:source_file] || File.join(SimplyCivi::SimplyCivi_ROOT_PATH, "screen.css")
    ...
    def css_from_assignments(assignments = {})
      ...
      SimplyCivi_assignments = CSSParser.new(File.path_to_string(self.source_file)).parse

      # iterates through each class assignment ('#footer' => '.span-24', '#header' => '.span-24')
      assignments.each do |semantic_class, SimplyCivi_classes|
        # gathers all BP classes we're going to be mimicing
        SimplyCivi_classes = SimplyCivi_classes.split(/,|\s/).select {|c| !c.blank? }.flatten.map {|c| c.strip }
        classes = []
        # loop through each BP class, grabbing the full hash (containing tags, index, and CSS rules)
        SimplyCivi_classes.each do |bp_class|
          match = if bp_class.include?(".")
                    bp_class.gsub(".", ".#{self.namespace}")
                  else
                    ".#{self.namespace}#{bp_class}"
                  end
          classes << SimplyCivi_assignments.select do |line|
