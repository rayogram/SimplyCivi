...
module SimplyCivi
  class Compressor
    ...
    # also sets custom_path flag on SimplyCivi::Compressor instance
    def destination_path=(path)
      @destination_path = path
      @custom_path = @destination_path != SimplyCivi::SimplyCivi_ROOT_PATH
    end
    ...
    def initialize
      ...
      @settings_file = SimplyCivi::SETTINGS_FILE
      self.namespace = ""
      self.destination_path = SimplyCivi::SimplyCivi_ROOT_PATH
    ...
    def generate!
      ...
      generate_css_files  # loops through SimplyCivi::CSS_FILES to generate output CSS
    ...
    def options #:nodoc:#
      OptionParser.new do |o|
        ...
        o.define_head "SimplyCivi Compressor"
        o.separator ""
        o.separator "options"
        ...
        o.on( "-nBP_NAMESPACE", "--namespace=BP_NAMESPACE", String,
              "Define a namespace prepended to all SimplyCivi classes (e.g. .your-ns-span-24)") { |ns| self.namespace = ns }
    ...
    def initialize_project_from_yaml(project_name = nil)
      ...
      if (project = projects[project_name]) # checks to see if project info is present
        self.destination_path = if self.destination_path == SimplyCivi::SimplyCivi_ROOT_PATH
                                  project["path"]
                                else
    ...
    def generate_css_files
      SimplyCivi::CSS_FILES.each do |output_file_name, css_source_file_names|
        css_output_path = File.join(destination_path, output_file_name)
        puts "\n    Assembling to #{custom_path ? css_output_path : "default SimplyCivi path"}"

        # CSS file generation
        css_output = css_file_header # header included on all three SimplyCivi-generated files
        css_output += "\n\n"
        ...
        css_source_file_names.each do |css_source_file|
          ...
          source_options = if self.custom_layout && css_source_file == "grid.css"
            ...
            File.path_to_string File.join(SimplyCivi::SOURCE_PATH, css_source_file)
          end

          css_output += SimplyCivi::CSSParser.new(source_options,
    ...
    def append_plugin_css(css, current_file_name)
      ...
      self.plugins.each do |plugin|
        plugin_file_specific  = File.join(SimplyCivi::PLUGINS_PATH, plugin, current_file_name)
        plugin_file_generic   = File.join(SimplyCivi::PLUGINS_PATH, plugin, "#{plugin}.css")
    ...
    def generate_tests
      ...
      test_files = Compressor::TEST_FILES.map do |file|
        File.join(SimplyCivi::TEST_PATH, *file.split(/\//))
      end
    ...
    def output_header
      ...
      puts "  #{"*" * 100}"
      puts "  **"
      puts "  **   SimplyCivi CSS Compressor"
      puts "  **"
...
%(/* -----------------------------------------------------------------------
 ...
 SimplyCivi CSS Framework 0.9
 http://SimplyCivicss.org

   * Copyright (c) 2007-Present. See LICENSE for more info.
   * See README for instructions on how to use SimplyCivi.
